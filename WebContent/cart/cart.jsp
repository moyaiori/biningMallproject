...
<script>
		...
		for (var i in checkBox){			
			checkBox[i].onchange = function(){
				if(this.checked == true){
					...
					for(var j=0; j<orderCount.length; j++){
						price = checkSelect(orderCount[j]);
					}
	...
	function checkSelect(standard){
		var total = 0;
		if(standard.parentNode.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.firstChild.checked){
			total += parseInt(standard.parentNode.nextSibling.nextSibling.firstChild.nodeValue.trim());
		}
		return total;
	}
